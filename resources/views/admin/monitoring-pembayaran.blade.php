    ...
    <div class="col-sm-6">
      <div class="d-flex gap-2 align-items-end">
        ...
        <form action="{{ route('monitoring-pembayaran.export-excel') }}" method="POST">
          ...
          <input type="hidden" name="jenis_tunjangan" id="export_jenis_tunjangan" value="{{ $jenisTunjangan ?? 'sptjm' }}">
        ...
        fetch("{{ route('monitoring-pembayaran.data') }}", {
          ...
          if(tbody) {
            ...
            if (currentJenis === 'tukin') {
                ...
                // Totals
                const t=data.totals||{};
                let sumCols = `<td class="text-end">${fmt(t.kotorTpd||0)}</td><td class="text-end">${fmt(t.pajakTpd||0)}</td>${tkc(t.kotorTkgb||0)}${tkc(t.pajakTkgb||0)}<td class="text-end">${fmt(t.bersihTpd||0)}</td>${tkc(t.bersihTkgb||0)}`;
                let sumColspan = 4;
