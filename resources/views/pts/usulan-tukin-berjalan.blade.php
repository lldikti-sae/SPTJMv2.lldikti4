	...
	<div class="col-12">
		...
		<div class="card md2-card mb-4">
			<div class="card-body px-4 pb-4 pt-4">
				...
				<form id="bulanForm" method="GET" action="{{ route('pts.usulan-tukin-berjalan') }}">
					<div class="row">
						<div class="col-md-4">
		...
		<div class="card md2-card mb-4">
			<div class="card-body px-4 pb-4 pt-4">
				...
				<div class="d-flex flex-wrap align-items-center justify-content-center gap-2 mt-4">
					...
					<!-- Upload + Usulkan (opsional untuk Tukin, tetap sediakan upload untuk konsistensi UI) -->
					  <div class="d-flex align-items-center flex-wrap gap-2">
						...
						<input type="file" id="fileInput" name="file" form="formUsulkanTukinBerjalan" accept="application/pdf" class="d-none" onchange="updateFileName(this)">

									<form id="formUsulkanTukinBerjalan" method="POST" action="{{ route('pts.usulkan-tukin-berjalan') }}" class="d-inline" enctype="multipart/form-data">
										@csrf
										...
										<button type="button" id="usulkanButton" class="btn btn-info" onclick="submitUsulanBerjalan()">
				...
				<div class="alert alert-info small mt-3">
					<ul class="mb-0 ps-3">
						<li><strong>Penamaan File: Tukin Berjalan_[Bulan Tahun]_[Nama PTS]</strong></li>
...
<script>
	...
	// hide Usulkan button until a file is chosen (match SPTJM behavior)
	try {
		...
		const data = {
			...
			localStorage.setItem('dataTukin', JSON.stringify(data));
		const url = "{{ route('pts.print-tukin-berjalan') }}" + "?bulan=" + bulan;
		window.open(url, '_blank');
	}

		function submitUsulanBerjalan() {
			...
			Swal.fire({
				...
				// Submit form
				document.getElementById('formUsulkanTukinBerjalan').submit();
			});
		}
