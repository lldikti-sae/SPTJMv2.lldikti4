...
class KekuranganBayarController extends Controller
{
  ...
  private function evaluateFullyPaidNidns($versi, $tarifMap = [], $targetNidn = null)
  {
      $fullyPaidNidns = [];
      if (empty($tarifMap)) {
      ...
      $activePayments = DB::table('t_kekurangan')
          ->where('tahun', $versi)
          ->where('jenis_pembayaran', 'like', 'PEMBAYARAN_%')
          ->get();
      foreach ($activePayments as $p) {
          $ident = !empty($p->nidn) ? $p->nidn : ($p->nuptk ?? null);
          if (!$ident) continue;
          $parts = explode('_', $p->jenis_pembayaran);
          $m = isset($parts[1]) ? (int) $parts[1] : 0;
          if ($m > 0) {
              if (!isset($paidKotorByNidnMonth[$ident][$m])) {
                  $paidKotorByNidnMonth[$ident][$m] = 0;
              }
              $paidKotorByNidnMonth[$ident][$m] += abs((float) $p->selisih);
          }
      }
      ...
      $rows = $rowsQuery->select(
                  'k.NIDN', 'k.NUPTK', 'k.Jenis', 'k.Jabatan12',
          ...
          foreach ($rows as $row) {
              $nidn = !empty($row->NIDN) ? $row->NIDN : ($row->NUPTK ?? '');
              ...
              for ($m = 1; $m <= 12; $m++) {
                  ...
                  if ($paidNet > 0) {
                      $paidGross = $paidNet; // It is already gross!
    ...
    private function getNidnsWithPayments($versi)
    {
    ...
    $transformer = function ($row) use ($tarifMap, $paidKotorByNidnMonth) {
      ...
      for ($i = 1; $i <= 12; $i++) {
        ...
        if ($sp2dOk) {
          ...
          if ($paidNet > 0) {
              // paidNet is already the gross amount since it's saved as gross in t_kekurangan now
              $paidGross = $paidNet;
  ...
  public function proses(Request $request)
  {
    ...
    try {
      ...
      foreach ($combinations as $combo) {
        ...
        // Exclude those who are already fully paid
        ...
        $select = [
          ...
          'k.Nama_Pegawai as Nama_Pegawai', 'k.No_Rekening as No_Rekening', 'k.Nama_Rekening as Nama_Rekening', 'k.NPWP as NPWP',
        ];
        for ($i = 1; $i <= 12; $i++) {
  ...
  public function prosesAksiSp2d(Request $request)
  {
    ...
    try {
      ...
      $rekapTipe = $rekap ? trim($rekap->tipe ?? 'Semua') : 'Semua';
      $rekapBank = $rekap ? trim($rekap->bank ?? 'Semua') : 'Semua';
      ...
      foreach ($dosenRows as $dosen) {
        ...
        if ($inputNominal !== null) {
            // For carry-over, add all other months
            for ($m = 1; $m <= 12; $m++) {
                if ($inputBulan && $m == (int)$inputBulan) continue;
                $monthsToProcess[] = $m;
            }
        } else {
            // No carry-over, only process inputBulan, or all if no inputBulan
            if (!$inputBulan) {
        ...
        foreach ($monthsToProcess as $i) {
          ...
          // Only process if debt sign matches the trxType!
          // Kurang Bayar -> selisihTotal is POSITIVE
          // Lebih Bayar -> selisihTotal is NEGATIVE
          ...
          // Hitung Total Hutang Bersih Asli
          ...
          if (stripos($jenisDsn, 'PNS') !== false && \Illuminate\Support\Facades\Schema::hasTable('t_uraian_pembayaran')) {
              if ($isKurang || $isLebih) {
                  $tukinAdjust = $isKurang ? round($paymentBersih, 2) : -round($paymentBersih, 2);
                  $uraianInsertBatch[] = [
                      'nidn' => $nidn,
                      'bersih' => $tukinAdjust,
      ...
      if ($nidnInput) {
          $transaksi = DB::table('s_transaksi_2')
              ->where(function ($q) use ($nidnInput) {
                  $q->where('NIDN', $nidnInput)
                    ->orWhere('NUPTK', $nidnInput);
              })
              ->where('Tahun_Versi', $versi)
              ->first();

          if ($transaksi && (float)($transaksi->Bersih_Selisih ?? 0) <> 0) {
              $isLunas = true;
          }
      }
  ...
  public function destroyKurang(Request $request)
  {
    ...
    $protectedNidns = array_unique(array_merge($rekapsKurangNidns, $fullyPaidNidns, $nidnsWithPayments));
  ...
  public function destroyLebih(Request $request)
  {
    ...
    list($fullyPaidNidns, ) = $this->evaluateFullyPaidNidns($versi);
    $nidnsWithPayments = $this->getNidnsWithPayments($versi);
    $protectedNidns = array_unique(array_merge($rekapsLebihNidns, $fullyPaidNidns, $nidnsWithPayments));
    ...
    public function getRiwayat(Request $request)
  {
      ...
      $riwayat = $riwayatRows->map(function($r) {
          $r->nomor = $r->kode_bayar_k ?? '';
          $r->tanggal = $r->tgl_bayar_k ?? '';
          $r->nominal = (float) ($r->selisih ?? 0);
          $r->uraian_pembayaran = $r->keterangan ?? '';
          
          $parts = explode('_', $r->jenis_pembayaran);
          $r->bulan = isset($parts[1]) ? (int)$parts[1] : 0;
          $r->bersih = abs($r->nominal); // Asumsi kotor = bersih jika tarif 0
          return $r;
      });
  ...
  public function detailRekap(Request $request, $id)
  {
      ...
      $select = [
          'k.NIDN as NIDN', 'k.NUPTK as NUPTK', 'k.Nama as Nama', 'k.Jenis as Jenis', 'k.Bank as Bank',
          'k.Jabatan12 as Jabatan12', 'k.Aktif as Aktif', 'ku.bersih as delta_bersih',
      ];
      for ($i = 1; $i <= 12; $i++) {
          $select[] = 'k.Gol' . $i; $select[] = 'k.Jabatan' . $i;
      ...
      if (!empty($excludeNidns)) {
          $base->where(function($q) use ($excludeNidns) {
              $q->whereNotIn('k.NIDN', $excludeNidns)
                ->whereNotIn('k.NUPTK', $excludeNidns);
          });
      }
      
      // JIKA REKAP BELUM DIPROSES SP2D, HILANGKAN DOSEN YANG SUDAH LUNAS SECARA INDIVIDU
      if (empty($rekap->sp2d)) {
          list($fullyPaidNidns, $paidKotorByNidnMonthDummy) = $this->evaluateFullyPaidNidns($versi);
          if (!empty($fullyPaidNidns)) {
              $base->where(function($q) use ($fullyPaidNidns) {
                  $q->whereNotIn('k.NIDN', $fullyPaidNidns)
                    ->whereNotIn('k.NUPTK', $fullyPaidNidns);
    ...
    public function updateRiwayat(Request $request)
  {
      ...
      $versi = session('tahun');
      if (!$versi) {
          return response()->json(['success' => false, 'message' => 'Tahun versi belum dipilih pada sesi.']);
      }
      ...
      if (!$riwayat) {
          return response()->json(['success' => false, 'message' => 'Data riwayat tidak ditemukan dalam database']);
      }

      // Ambil transaksi untuk hitung ulang pajak/kotor dan mendapatkan JSON
      $transaksi = DB::table('s_transaksi_2')
          ->where(function ($q) use ($nidn) {
              $q->where('NIDN', $nidn)
                ->orWhere('NUPTK', $nidn);
      ...
      $this->syncLunasToTransaksi($nidn, $versi);

      return response()->json(['success' => true, 'message' => 'Data riwayat berhasil diupdate']);
  }

  public function syncAllLunas(Request $request)
  {
      $versi = session('tahun');
      list($fullyPaidNidns, ) = $this->evaluateFullyPaidNidns($versi);

      $count = 0;
      foreach ($fullyPaidNidns as $nidn) {
          $this->syncLunasToTransaksi($nidn, $versi);
          $count++;
      }

      return redirect()->back()->with('success', "Berhasil menyinkronkan $count data selesai ke tabel transaksi utama.");
  }
  ...
  private function syncLunasToTransaksi($nidn, $versi)
  {
      ...
      if (in_array($nidn, $fullyPaidNidns)) {
          ...
          $originalSelisih = DB::table('t_kekurangan')
              ...
              ->where('jenis_pembayaran', 'not like', 'PEMBAYARAN_%')
              ->sum('selisih');

          // Balikkan nilainya agar Kurang (+) dan Lebih (-) sesuai ekspektasi user
          // Karena di database t_kekurangan, Kurang = negatif, Lebih = positif
          $originalSelisih = $originalSelisih * -1;

          // Nilai dari t_kekurangan adalah nilai KOTOR
          $kotorSelisih = $originalSelisih;
          ...
          if ($transaksi) {
              $gol = $transaksi->Gol1 ?? $transaksi->Gol2 ?? $transaksi->Gol3 ?? '';
              $jenis = $transaksi->Jenis ?? '';
              $tarifMap = $this->loadTarifPajakMap();
              $tarif = (float) (($tarifMap[trim($jenis)][trim($gol)] ?? 0) ?: 0);
              
              // Hitung Pajak dan Bersih dari nilai Kotor
              $pajakSelisih = $kotorSelisih * $tarif;
              $bersihSelisih = $kotorSelisih - $pajakSelisih;

              // Simpan ke s_transaksi_2
              DB::table('s_transaksi_2')
                  ->where(function ($q) use ($nidn) {
                      $q->where('NIDN', $nidn)->orWhere('NUPTK', $nidn);
                  })
                  ->where('Tahun_Versi', $versi)
                  ->update([
                      'Kotor_Selisih' => round($kotorSelisih, 2),
                      'Pajak_Selisih' => round($pajakSelisih, 2),
                      'Bersih_Selisih' => round($bersihSelisih, 2),
                  ]);
          }
      } else {
          // Jika belum lunas atau batal lunas, bersihkan kolomnya
          DB::table('s_transaksi_2')
              ->where(function ($q) use ($nidn) {
                  $q->where('NIDN', $nidn)->orWhere('NUPTK', $nidn);
              })
