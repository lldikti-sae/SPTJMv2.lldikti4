...
class LaporanKeuanganPicExport implements FromGenerator, WithHeadings, WithTitle, WithEvents
{
  protected $kode_pt;
  protected $tahun;
  protected $nidn;
  ...
  public function __construct($kode_pt, $tahun = null, $nidn = null, $emailPIC = null)
  {
    $this->kode_pt = $kode_pt;
    $this->tahun = $tahun ?: date('Y');
    $this->nidn = $nidn;
    $this->emailPIC = $emailPIC ? trim((string) $emailPIC) : null;
  }
  ...
  public function generator(): \Generator
  {
    ...
    $select = [
      ...
      DB::raw("COALESCE(`t`.`{$jabatanCol}`, `t`.`Jabatan12`, '-') AS jabatan"),
    ...
    $query = DB::table('s_transaksi_2 as t')
      ...
      ->when($this->emailPIC, function($q) {
        $q->whereRaw('TRIM(`t`.`Pemegang_Wilayah`) = ?', [$this->emailPIC]);
      });
    ...
    foreach ($query->cursor() as $d) {
      ...
      for ($i = 1; $i <= 12; $i++) {
        ...
        $totalGaji += $monthlyGaji;
        $totalTPD += $tpd;
        $totalTKGB += $tkgb;

        $totalGajiPerMonth[$i - 1] += $monthlyGaji;
        $totalTpdPerMonth[$i - 1] += $tpd;
        $totalTkgbPerMonth[$i - 1] += $tkgb;
        $grandTotalGaji += $monthlyGaji;
        $grandTotalTPD += $tpd;
        $grandTotalTKGB += $tkgb;
